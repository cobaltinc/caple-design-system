...
export default ({ children, multiple = false, onChange, className = '', style }: SwitchGroupProps) => {
  ...
  const convertReactNodeToSwitchs = (reactNode: React.ReactNode) => (
    convertReactNodeTo<SwitchProps>('Switch.Group', 'Switch', reactNode)
  ...
  let switchs = convertReactNodeToSwitchs(children);
    const [checkes, setCheckes] = useState<boolean[]>(switchs.map((_, index) => {
          ...
          .map((element, index) => {
            ...
            return (
              <label className={`${switchClassPrefix}--container`} key={index}>
                ...
                <button className={classnames(switchClassPrefix, props.className)} style={props.style} disabled={!!props.disabled} onClick={() => onClick(index)}>
                  {props.title}
                </button>
              </label>
            );
